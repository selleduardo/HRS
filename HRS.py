...
class MyWindow(Gtk.Window):
    ...
    def CalculaMediaMaximo(self, signal):
    
        sinal = np.zeros(30)

        for array in signal.T:
            Idx = np.where(array == max(array))[0]
            Idx = Idx[int(len(Idx)/2.)]# + 1

            sobe = array[:Idx][::-1]
            sinal = np.column_stack((sinal, sobe[:30]))
            # plt.plot(sinal)

        sinal = sinal[:, 1:]
        sinal = np.column_stack((
            np.average(sinal, axis = 1),
            np.std(sinal, axis = 1)/np.sqrt(10)))   
    ...
    def plota(self):
        ...
        self.sgn = self.sgn_temp[self.IM:, :][::-1] - self.medsgn
        self.ref = self.ref_temp[self.IM:][::-1] - self.medref

        self.sgn[:, 0] = (2*self.sgn[:, 0])/(10**(-self.A532*0.5) + 10**(-self.A532*1.5))

        # self.sgn = self.CalculaMediaMaximo(self.Imps)
        # self.referencia = self.CalculaMediaMaximo(self.Impr)
        # self.ref = self.referencia[:, 0]
        self.IM = -1
